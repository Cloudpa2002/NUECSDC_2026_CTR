...
#include <opencv2/videoio/videoio_c.h>  // 添加头文件
...
int main(int argc, char **argv)
{
    ...
    if (is_camera_id) {
        ...
        // 方法1：直接打开摄像头 ID（可能不稳定）
        // cap.open(camera_id, cv::CAP_V4L2);
        
        // 方法2：通过 /dev/videoX 路径打开（推荐）
        // std::string v4l2_device = "/dev/video" + std::to_string(camera_id);
        // cap.open(v4l2_device, cv::CAP_V4L2);
