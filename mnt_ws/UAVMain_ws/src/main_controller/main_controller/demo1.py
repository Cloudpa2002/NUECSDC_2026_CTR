...
def main(args=None):
    ...
    input_value_aim = 1  # 默认值为 0
    input_value_pre = 1  # 默认值为 0
    if len(sys.argv) > 1:  # 检查是否有额外参数
        try:
            # 获取传入的整型参数
            # 第一个参数是目标颜色处
            # 第二个参数是先飞到的好规划的颜色处
            input_value_aim = int(sys.argv[1])
            input_value_pre = int(sys.argv[2])
            print(f"Input value for aim_color: {input_value_aim}")
            print(f"Input value for pre_aim_color: {input_value_pre}")
        except ValueError:
    ...
    while rclpy.ok():
        try:
            ...
            if mask == 1:
                ...
                # module1(node)
                node.color = input_value_aim if len(sys.argv) > 1 else 1  # 使用传入的参数或默认值
                node.pre_aim = input_value_pre if len(sys.argv) > 2 else 1  # 使用传入的参数或默认值
            ...
            if mask == 3:
                # 发布目标位置到 /move_base_simple/goal
                time.sleep(3)
                print(f"Module 3: 发布目标位置: {node.destination[node.pre_aim]}")
                node.publish_goal(node.destination[node.pre_aim],position_th=0.15)
                time.sleep(1)
