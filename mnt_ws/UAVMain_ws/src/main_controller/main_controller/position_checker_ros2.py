...
def is_position_reached(node, target_position, position_threshold=0.03, odom_topic="/Odometry"):
    """
    判断当前位置是否到达目标位置
    参数：
        node: ROS2 节点实例
    ...
    class PositionChecker(Node):
        def __init__(self):
            super().__init__('position_checker_temp_node')
            self.target_pos = np.array(target_position)  # 只取 x 和 y
            self.position_th = position_threshold
            self.current_pos = None
            self.odom_sub = self.create_subscription(
                Odometry,
        ...
        def check_position(self):
            # 如果当前位置未更新，返回 False
            if self.current_pos is None:
                self.get_logger().warn("当前位置未更新，无法判断是否到达目标位置")
                return False

            # 计算位置距离（只计算 x 和 y）
            pos_dist = np.linalg.norm(self.current_pos[:2] - self.target_pos[:2])
            self.get_logger().info(f"Distance: {pos_dist:.2f}m ，current_pos = {self.current_pos}")
            return pos_dist < self.position_th

    # 初始化 PositionChecker 节点
    checker = PositionChecker()
