...
from tools.tof_sensor import wait_until_closer_than
import time
import subprocess
import numpy as np
...
class Mask:
    ...
    此类包含所有 mask 的具体定义
    """
    def __init__(self, node, target_pos_on=None,target_pos1=None,target_pos1_z=None,target_pos1_1=None,target_pos1_2=None,target_pos1_3=None, target_pos2=None, target_pos2_1=None,
                 target_pos2_2=None, target_pos3=None, target_pos3_1=None, target_pos3_2=None, target_pos3_3=None, target_pos4=None, target_pos_preland=None, target_pos_hover=None):
        """
        功能: 初始化
        输入: node - Initialization 节点实例
             target_pos1 - list [x, y, z]，圆环1
             target_pos1_3 - list [x, y, z]，圆环2前升高度
             target_pos_on - list [x, y, z]，起飞点上面
             target_pos2 - list [x, y, z]，圆环2后
             target_pos2_1 - list [x, y, z]，移动障碍物前
             target_pos3 - list [x, y, z]，圆环3前
             target_pos4 - list [x, y, z]，圆环4
             target_pos_preland - list [x, y, z]，预降落点
             target_pos_hover - list [x, y, z]，起飞悬停点
        输出: 无
        """
        self.node = node
        self.target_pos_on = target_pos_on
        self.target_pos1 = target_pos1
        self.target_pos1_z = target_pos1_z
        self.target_pos1_1 = target_pos1_1
        self.target_pos1_2 = target_pos1_2
        self.target_pos1_3 = target_pos1_3
        self.target_pos2 = target_pos2
        self.target_pos2_1 = target_pos2_1
        self.target_pos2_2 = target_pos2_2
        self.target_pos3 = target_pos3
        self.target_pos3_1 = target_pos3_1
        self.target_pos3_2 = target_pos3_2
        self.target_pos3_3 = target_pos3_3
        self.target_pos4 = target_pos4
    ...
    def mask2(self):
        """
        功能: 依次前往所有目标点（圆环1→圆环2后→移动障碍物前→圆环3前→圆环4→预降落点），每一步到达后才发布下一个
        输入: 无
        ...
        # 前往起飞点上方
        if not self.node.set_planner_mode('ONE'):
            return False
        self.node.get_logger().info(f"前往起飞点上方: {self.target_pos_on}")
        if not self.pos_control.publish_goal(self.target_pos_on, position_th=0.2, velocity_th=1.0):
            self.node.get_logger().error("前往起飞点上方失败")
            return False
        ...
        if not self.node.set_planner_mode('ONE'):
            return False
        self.node.get_logger().info(f"前往圆环1前: {self.target_pos1}")
        if not self.pos_control.publish_goal(self.target_pos1, position_th=0.2, velocity_th=1.0):
            self.node.get_logger().error("前往圆环1前失败")
            return False
        
        # 前往圆环1后
        if not self.node.set_planner_mode('NORMAL'):
            return False
        time.sleep(1.0)
        self.node.get_logger().info(f"前往圆环1后: {self.target_pos1_z}")
        if not self.pos_control.publish_goal(self.target_pos1_z, position_th=0.2, velocity_th=1.0):
            self.node.get_logger().error("前往圆环1后失败")
            return False
        
        # 林区降高度
        if not self.node.set_planner_mode('NORMAL'):
            return False
        self.node.get_logger().info(f"林区降高度: {self.target_pos1_1}")
        if not self.pos_control.publish_goal(self.target_pos1_1, position_th=0.2, velocity_th=1.0):
            self.node.get_logger().error("林区降高度失败")
            return False
        
        # 前往圆环2前
        if not self.node.set_planner_mode('TWO'):
            return False
        time.sleep(1.0)
        self.node.get_logger().info(f"前往圆环2前: {self.target_pos1_2}")
        if not self.pos_control.publish_goal(self.target_pos1_2, position_th=0.2, velocity_th=1.0):
            self.node.get_logger().error("前往圆环2前失败")
            return False

        if not self.node.set_planner_mode('NORMAL'):
            return False
        # 前往圆环2前升高度
        self.node.get_logger().info(f"前往圆环2前升高度: {self.target_pos1_3}")
        if not self.pos_control.publish_goal(self.target_pos1_3, position_th=0.2, velocity_th=1.0):
            self.node.get_logger().error("前往圆环2前升高度失败")
            return False

        if not self.node.set_planner_mode('NORMAL'):
            return False
        self.node.get_logger().info(f"前往圆环2后: {self.target_pos2}")
        if not self.pos_control.publish_goal(self.target_pos2, position_th=0.2, velocity_th=1.0):
            self.node.get_logger().error("前往圆环2后失败")
            return False

        # 前往移动障碍物1前
        if not self.node.set_planner_mode('NORMAL'):
            return False
        self.node.get_logger().info(f"前往移动障碍物前: {self.target_pos2_1}")
        if not self.pos_control.publish_goal(self.target_pos2_1, position_th=0.2, velocity_th=0.3):
            self.node.get_logger().error("前往移动障碍物前失败")
            return False

        # 前往移动障碍物2前
        if not self.node.set_planner_mode('THREE'):
            return False
        if not wait_until_closer_than(1.0):
            return False
        self.node.get_logger().info(f"前往移动障碍物2前: {self.target_pos2_2}")
        if not self.pos_control.publish_goal(self.target_pos2_2, position_th=0.2, velocity_th=0.3):
            self.node.get_logger().error("前往移动障碍物2前失败")
            return False
        ...
        if not wait_until_closer_than(1.0):
            return False
        self.node.get_logger().info(f"前往圆环3前: {self.target_pos3}")
        if not self.pos_control.publish_goal(self.target_pos3, position_th=0.2, velocity_th=1.0):
            self.node.get_logger().error("前往圆环3前失败")
            return False

        # 前往圆环3后
        if not self.node.set_planner_mode('NORMAL'):
            return False
        self.node.get_logger().info(f"前往圆环3后: {self.target_pos3_1}")
        if not self.pos_control.publish_goal(self.target_pos3_1, position_th=0.2, velocity_th=0.3):
            self.node.get_logger().error("前往圆环3后失败")
            return False
        ...
        if not self.node.set_planner_mode('FOUR'):
            return False
        time.sleep(1.0)
        self.node.get_logger().info(f"前往拱门前: {self.target_pos3_2}")
        if not self.pos_control.publish_goal(self.target_pos3_2, position_th=0.2, velocity_th=0.3):
            self.node.get_logger().error("前往拱门前失败")
            return False
        
        # 前往拱门后
        if not self.node.set_planner_mode('FOUR'):
            return False
        self.node.get_logger().info(f"前往拱门后: {self.target_pos3_3}")
        if not self.pos_control.publish_goal(self.target_pos3_3, position_th=0.2, velocity_th=0.3):
            self.node.get_logger().error("前往拱门后失败")
            return False
        # 前往圆环4前
        if not self.node.set_planner_mode('FOUR'):
            return False
        self.node.get_logger().info(f"前往圆环4: {self.target_pos4}")
        if not self.pos_control.publish_goal(self.target_pos4, position_th=0.2, velocity_th=1.0):
            self.node.get_logger().error("前往圆环4失败")
            return False
...
def main(args=None):
    ...
    target_pos_on = [0.0, 0.0, 0.6]        # 起飞点上面
    target_pos1 = [4.00, 0.43, 1.14]    # 圆环1前
    target_pos1_z = [5.20, 0.43, 1.14]  # 圆环1后
    target_pos1_1 = [target_pos1_z[0], target_pos1_z[1], 0.3]  # 林区降高度
    target_pos1_2 = [8.57, -1.24, target_pos1_1[2]]  # 圆环2前
    target_pos1_3 = [9.00, -1.89, 1.14]  # 圆环2前升高度
    target_pos2 = [9.00, -2.88, 1.14]    # 圆环2后
    target_pos2_1 = [8.00, -3.85, 1.14]    # 移动障碍物1前
    target_pos2_2 = [6.19, target_pos2_1[1], 1.14]    # 移动障碍物2前
    target_pos3 = [4.51, -3.73, 1.14]    # 圆环3前
    target_pos3_1 = [3.40, target_pos3[1], 1.14]    # 圆环3后
    target_pos3_2 = [3.18, -3.05, 1.14]    # 拱门前
    target_pos3_3 = [1.69, target_pos3_2[1], 1.14]    # 拱门后
    target_pos4 = [1.22, -3.76, 1.14]    # 圆环4
    target_pos_preland = [-0.12, -3.81, 1.14]    # 预降落点

    mask = Mask(node, target_pos_on=target_pos_on, target_pos1=target_pos1, target_pos1_z=target_pos1_z,target_pos1_1=target_pos1_1,
                target_pos1_2=target_pos1_2, target_pos1_3=target_pos1_3, target_pos2=target_pos2,
                target_pos2_1=target_pos2_1, target_pos2_2=target_pos2_2, target_pos3=target_pos3,
                target_pos3_1=target_pos3_1,target_pos3_2=target_pos3_2,
                target_pos3_3=target_pos3_3,
                target_pos4=target_pos4, target_pos_preland=target_pos_preland,
                target_pos_hover=target_pos_hover)
    current = 0
