...
class UAV_Status:
    ...
    def run_force_disarm_command_in_home(self):
        """
        功能: 调用 /mavros/cmd/command 服务，用于强制关闭无人机电门
        输入: 无
        输出: bool - True 表示强制关闭电门成功，False 表示失败
        """
        self.node.get_logger().info('准备调用 /mavros/cmd/command 服务强制关闭电门')

        if not self.node.command_client.wait_for_service(timeout_sec=2.0):
            self.node.get_logger().error('/mavros/cmd/command 服务不可用')
            return False
        ...
        future = self.node.command_client.call_async(request)
        rclpy.spin_until_future_complete(self.node, future, timeout_sec=3.0)

        if not future.done():
            self.node.get_logger().error('强制关闭电门超时')
            return False

        response = future.result()
        if response is None:
            self.node.get_logger().error('强制关闭电门失败，无响应')
            return False

        if response.success:
            self.node.get_logger().info(f'强制关闭电门成功: result={response.result}')
        else:
            self.node.get_logger().error(f'强制关闭电门失败: result={response.result}')
...
class Mask:
    ...
    此类包含所有 mask 的具体定义
    """
    def __init__(self, node, target_pos1=None, target_pos2=None, target_pos3=None, target_pos_hover=None):
        """
        功能: 初始化
        输入: node - Initialization 节点实例
             target_pos1 - list [x, y, z]，目标点
             target_pos2 - list [x, y, z]，预降落点
             target_pos3 - list [x, y, z]，降落点
    ...
    def mask2(self):
        """
        功能: 依次前往目标点、预降落点、降落点，每一步到达后才发布下一个
        输入: 无
        ...
        if not self.node.set_planner_mode('NORMAL'):
            return False
        self.node.get_logger().info(f"前往目标点: {self.target_pos1}")
        if not self.pos_control.publish_goal(self.target_pos1, position_th=0.2):
            self.node.get_logger().error("前往目标点失败")
            return False

        # 前往预降落点
        if not self.node.set_planner_mode('NORMAL'):
            return False
        self.node.get_logger().info(f"前往预降落点: {self.target_pos2}")
        if not self.pos_control.publish_goal(self.target_pos2, position_th=0.15):
            self.node.get_logger().error("前往预降落点失败")
            return False

        # 前往降落点
        self.node.get_logger().info(f"前往降落点: {self.target_pos3}")
        goal = PoseStamped()
        goal.header.frame_id = "world"
        goal.pose.position.x = self.target_pos3[0]
        goal.pose.position.y = self.target_pos3[1]
        goal.pose.position.z = self.target_pos3[2]
        goal.pose.orientation.w = 1.0

        for _ in range(20):
            self.node.goal_publisher.publish(goal)
            self.node.get_logger().info(f'已发布目标位置: x={self.target_pos3[0]}, y={self.target_pos3[1]}, z={self.target_pos3[2]}')
    ...
    def mask4(self):
        if not self.status.run_force_disarm_command_in_home():
            self.node.get_logger().error("强制关闭电门失败")
            return False
        return True
...
def main(args=None):
    ...
    target_pos1 = [1.96, -0.02, 1.0]    # 目标点
    target_pos2 = [1.96, -0.02, 0.1]  # 预降落点
    target_pos3 = [1.96, -0.02, -0.8]  # 降落点

    mask = Mask(node, target_pos1=target_pos1, target_pos2=target_pos2,
                target_pos3=target_pos3, target_pos_hover=target_pos_hover)
    current = 0
