...
class Mask:
    ...
    """
    def __init__(self, node, target_pos_preland=None, target_pos_1=None, target_pos_2=None, target_pos_hover=None):
        """
        功能: 初始化
        输入: node - Initialization 节点实例
             target_pos_preland - list [x, y, z]，预降落点
             target_pos_1 - list [x, y, z]，第一个目标点
             target_pos_2 - list [x, y, z]，第二个目标点
    ...
    def mask2(self):
        ...
        self.node.get_logger().info(f"前往目标点1: {self.target_pos_1}")
        if not self.pos_control.publish_goal(self.target_pos_1, position_th=0.2):
            self.node.get_logger().error("前往目标点1失败")
            return False
        
        if not wait_until_closer_than(1.0):
            return False
        self.node.get_logger().info(f"前往目标点2: {self.target_pos_2}")
        if not self.pos_control.publish_goal(self.target_pos_2, position_th=0.2):
            self.node.get_logger().error("前往目标点2失败")
            return False
...
def main(args=None):
    ...
    target_pos_1 = [-0.6, 0.0, 0.6]
    target_pos_2 = [-2.3, 0.0, 0.6]
    target_pos_preland = [-4.1, 0.0, 0.6]    # 预降落点

    mask = Mask(node, target_pos_preland=target_pos_preland, target_pos_1=target_pos_1, target_pos_2=target_pos_2, target_pos_hover=target_pos_hover)
    current = 0
