...
class Mask:
    ...
    此类包含所有 mask 的具体定义
    """
    def __init__(self, node, target_pos1=None,target_pos1_z=None,target_pos1_1=None,target_pos1_2=None, target_pos2=None, target_pos2_1=None,
                 target_pos2_2=None, target_pos3=None, target_pos3_1=None, target_pos3_2=None,target_pos4=None, target_pos_preland=None, target_pos_hover=None):
        """
        功能: 初始化
        输入: node - Initialization 节点实例
    ...
    def mask2(self):
        ...
        if not self.node.set_planner_mode('NORMAL'):
            return False
        time.sleep(1.0)
        self.node.get_logger().info(f"前往圆环1中心: {self.target_pos1_z}")
        if not self.pos_control.publish_goal(self.target_pos1_z, position_th=0.2, velocity_th=1.0):
            self.node.get_logger().error("前往圆环1中心失败")
            return False
        
        # 前往圆环1后
        self.node.get_logger().info(f"前往圆环1后: {self.target_pos1_1}")
...
def main(args=None):
    ...
    target_pos1 = [3.94, 0.47, 1.14]    # 圆环1前
    target_pos1_z = [4.94, 0.47,1.14]  # 圆环1中心
    target_pos1_1 = [5.48, 0.47, 0.5]  # 圆环1后
    target_pos1_2 = [8.57, -1.24, 0.5]  # 圆环2前
    target_pos2 = [9.04, -2.46, 1.14]    # 圆环2后
    target_pos2_1 = [8.48, -3.52, 1.14]    # 移动障碍物1前
    target_pos2_2 = [8.48, -3.52, 1.14]    # 移动障碍物2前
    target_pos3 = [4.70, -3.65, 1.14]    # 圆环3前
    target_pos3_1 = [3.49, -3.65, 1.14]    # 圆环3后
    target_pos3_2 = [2.88, -3.11, 1.14]    # 拱门前
    target_pos4 = [1.22, -3.76, 1.14]    # 圆环4
    target_pos_preland = [-0.12, -3.81, 1.14]    # 预降落点

    mask = Mask(node, target_pos1=target_pos1, target_pos1_z=target_pos1_z,target_pos1_1=target_pos1_1,
                target_pos1_2=target_pos1_2, target_pos2=target_pos2,
