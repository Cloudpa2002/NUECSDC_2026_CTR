...
import subprocess
import numpy as np
from tools.ground_circle_detector import ground_circle_detector1
...
class Mask:
    ...
    此类包含所有 mask 的具体定义
    """
    def __init__(self, node, target_pos1=None, target_pos_hover=None):
        """
        功能: 初始化
        输入: node - Initialization 节点实例
             target_pos1 - list [x, y, z]，路径点1
...
def main(args=None):
    ...
    target_pos_hover = [0.0, 0.0, 1.1]     # 起飞悬停点
    target_pos1 = [2.0, 0.0, 1.1]    # 路径点1

    mask = Mask(node, target_pos1, target_pos_hover)
    current = 0
    ...
    while rclpy.ok():
        try:
            if current == 0:
                ...
                if mask.mask3():
                    current += 1
                    node.get_logger().error("Mask 3（降落）完成")
                else:
                    node.get_logger().error("Mask 3（降落）失败，终止任务")
                    break
