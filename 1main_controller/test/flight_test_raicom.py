...
class Mask:
    ...
    def mask2(self):
        ...
        if not self.pos_control.publish_goal([1.83, 0.90, 1.45], position_th=0.2):
            return False
        
        if not self.pos_control.publish_goal([2.75, 1.88, 1.45], position_th=0.2):
            return False
        ...
        if not self.pos_control.publish_goal(self.target_pos_deploy, position_th=0.1):
...
def main(args=None):
    ...
    target_pos1 = [1.83, 0.0, 1.45]    # 路径点1
    target_pos2 = [2.53, 2.71, 1.53]    # 路径点2
    target_pos_deploy = [0.37, 2.50, 1.53]    # 预投放点
    target_pos_letter_A = [0.37, 1.64, 1.51]   # 字母A点位
    target_pos_letter_B = [0.37, 2.69, 1.51]   # 字母B点位
    target_pos_preland = [0.0, 0.0, 0.1]      # 预降落点
