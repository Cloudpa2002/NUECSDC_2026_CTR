...
#include "ego_planner/srv/set_planner_mode.hpp"
...
namespace ego_planner
{
  ...
  private:
    ...
    struct PlannerModeConfig
    {
      double max_vel;
      double max_acc;
      double ctrl_pt_dist;
      double planning_horizon;
      double optimizer_dist0;
    ...
    std::map<std::string, PlannerModeConfig> planner_mode_configs_;

    /* planning data */
    bool have_trigger_, have_target_, have_odom_, have_new_target_, have_recv_pre_agent_;
    FSM_EXEC_STATE exec_state_;
    // 订阅数据
    rclcpp::Publisher<std_msgs::msg::Int32>::SharedPtr exec_state_pub_;
    ...
    rclcpp::Service<ego_planner::srv::SetPlannerMode>::SharedPtr set_planner_mode_srv_;

    /* helper functions */
    void declareAndLoadPlannerModeConfigs();
    void loadPlannerModeConfig(const std::string &mode,
                               const std::string &param_name,
                               const PlannerModeConfig &default_config);
    bool applyPlannerMode(const std::string &mode, std::string &message);
