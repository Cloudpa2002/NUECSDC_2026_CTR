...
def generate_launch_description():
    ...
    max_vel = LaunchConfiguration('max_vel', default=0.5) # 最大速度 
    max_acc = LaunchConfiguration('max_acc', default=2.0) # 最大加速度
    ...
    obj_num_set = LaunchConfiguration('obj_num_set', default=1)
    ...
    ego_planner_node = Node(
        ...
        parameters=[
            ...
            {'grid_map/obstacles_inflation': 0.255},    # 障碍物膨胀半径
            {'grid_map/column_fill_enabled': True},
            {'grid_map/column_fill_max_height': 2.0},
            {'grid_map/cloud_inflate_z_step': 1},
            ...
            {'grid_map/virtual_ceil_height': 1.4},      #虚拟天花板高度
            ...
            {'optimization/lambda_smooth': 1.0},	# 平滑权重
            {'optimization/lambda_collision': 1.0},	# 避障权重
            {'optimization/lambda_feasibility': 0.1},
            {'optimization/lambda_fitness': 1.0},
            {'optimization/dist0': 0.2}, 		#静态障碍物距离
            ...
            # 运行模式参数
            #通用默认
            {'planner_modes/normal/max_vel': 0.5},
            {'planner_modes/normal/max_acc': 2.0},
            {'planner_modes/normal/control_points_distance': 0.2},
            {'planner_modes/normal/planning_horizon': 4.0},
            {'planner_modes/normal/dist0': 0.2},
            {'planner_modes/normal/obstacles_inflation': 0.17},
            {'planner_modes/normal/virtual_ceil_height': 1.4},
            {'planner_modes/normal/column_fill_enabled': True},
            {'planner_modes/normal/column_fill_max_height': 1.0},
            {'planner_modes/normal/cloud_inflate_z_step': 0},
            #区域1
            {'planner_modes/one/max_vel': 0.55},
            {'planner_modes/one/max_acc': 3.0},
            {'planner_modes/one/control_points_distance': 0.2}, #B-spline 初始控制点之间的目标间距
            {'planner_modes/one/planning_horizon': 4.0}, #局部规划前瞻距离
            {'planner_modes/one/dist0': 0.25},
            {'planner_modes/one/obstacles_inflation': 0.255},
            {'planner_modes/one/virtual_ceil_height': 1.4}, #虚拟天花板高度，低于 -0.5 表示关闭
            {'planner_modes/one/column_fill_enabled': True},
            {'planner_modes/one/column_fill_max_height': 2.0}, #柱体填充高度
            {'planner_modes/one/cloud_inflate_z_step': 0}, #点云障碍物在 z 方向上下膨胀几格
            #区域2
            {'planner_modes/two/max_vel': 0.5},
            {'planner_modes/two/max_acc': 2.0},
            {'planner_modes/two/control_points_distance': 0.2},
            {'planner_modes/two/planning_horizon': 4.0},
            {'planner_modes/two/dist0': 0.2},
            {'planner_modes/two/obstacles_inflation': 0.255},
            {'planner_modes/two/virtual_ceil_height': 0.6},
            {'planner_modes/two/column_fill_enabled': True},
            {'planner_modes/two/column_fill_max_height': 2.0},
            {'planner_modes/two/cloud_inflate_z_step': 0},
            #区域3
            {'planner_modes/three/max_vel': 0.8},
            {'planner_modes/three/max_acc': 2.0},
            {'planner_modes/three/control_points_distance': 0.2},
            {'planner_modes/three/planning_horizon': 4.0},
            {'planner_modes/three/dist0': 0.2},
            {'planner_modes/three/obstacles_inflation': 0.255},
            {'planner_modes/three/virtual_ceil_height': 1.8},
            {'planner_modes/three/column_fill_enabled': True},
            {'planner_modes/three/column_fill_max_height': 2.0},
            {'planner_modes/three/cloud_inflate_z_step': 0},
            #区域4
            {'planner_modes/four/max_vel': 0.5},
            {'planner_modes/four/max_acc': 2.0},
            {'planner_modes/four/control_points_distance': 0.2},
            {'planner_modes/four/planning_horizon': 4.0},
            {'planner_modes/four/dist0': 0.2},
            {'planner_modes/four/obstacles_inflation': 0.17},
            {'planner_modes/four/virtual_ceil_height': 1.4},
            {'planner_modes/four/column_fill_enabled': True},
            {'planner_modes/four/column_fill_max_height': 1.2},
            {'planner_modes/four/cloud_inflate_z_step': 0},
