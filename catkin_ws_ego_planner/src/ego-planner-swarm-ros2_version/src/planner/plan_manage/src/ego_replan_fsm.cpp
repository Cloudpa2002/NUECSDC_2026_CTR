...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    // 发布当前状态
    exec_state_pub_ = node_->create_publisher<std_msgs::msg::Int32>("exec_state", 10);
    ...
    declareAndLoadPlannerModeConfigs();

    using SetPlannerMode = ego_planner::srv::SetPlannerMode;
    set_planner_mode_srv_ = node_->create_service<SetPlannerMode>(
        "/set_planner_mode",
        [this](const std::shared_ptr<SetPlannerMode::Request> request,
               std::shared_ptr<SetPlannerMode::Response> response)
        {
          response->success = applyPlannerMode(request->mode, response->message);
          if (response->success)
          {
  ...
  void EGOReplanFSM::declareAndLoadPlannerModeConfigs()
  {
    double default_virtual_ceil_height = -0.1;
    node_->get_parameter("grid_map/virtual_ceil_height", default_virtual_ceil_height);

    PlannerModeConfig normal{
        ...
        0.2,
        0.25,
        default_virtual_ceil_height,
        true,
        2.0,
        1};

    loadPlannerModeConfig("ONE", "one", {0.8, 2.0, 0.25, 4.5, 0.18, 0.20, default_virtual_ceil_height, true, 2.0, 1});
    loadPlannerModeConfig("TWO", "two", normal);
    loadPlannerModeConfig("THREE", "three", {0.35, 1.5, 0.15, 3.0, 0.12, 0.12, default_virtual_ceil_height, true, 2.0, 1});
    loadPlannerModeConfig("FOUR", "four", {0.30, 1.5, 0.20, 3.0, 0.25, 0.30, default_virtual_ceil_height, true, 2.0, 1});
    loadPlannerModeConfig("NORMAL", "normal", normal);
  }

  void EGOReplanFSM::loadPlannerModeConfig(const std::string &mode,
                                           const std::string &param_name,
                                           const PlannerModeConfig &default_config)
  {
    PlannerModeConfig config = default_config;
    const std::string prefix = "planner_modes/" + param_name + "/";

    node_->declare_parameter(prefix + "max_vel", config.max_vel);
    node_->declare_parameter(prefix + "max_acc", config.max_acc);
    node_->declare_parameter(prefix + "control_points_distance", config.ctrl_pt_dist);
    node_->declare_parameter(prefix + "planning_horizon", config.planning_horizon);
    node_->declare_parameter(prefix + "dist0", config.optimizer_dist0);
    node_->declare_parameter(prefix + "obstacles_inflation", config.obstacles_inflation);
    node_->declare_parameter(prefix + "virtual_ceil_height", config.virtual_ceil_height);
    node_->declare_parameter(prefix + "column_fill_enabled", config.column_fill_enabled);
    node_->declare_parameter(prefix + "column_fill_max_height", config.column_fill_max_height);
    node_->declare_parameter(prefix + "cloud_inflate_z_step", config.cloud_inflate_z_step);

    node_->get_parameter(prefix + "max_vel", config.max_vel);
    node_->get_parameter(prefix + "max_acc", config.max_acc);
    node_->get_parameter(prefix + "control_points_distance", config.ctrl_pt_dist);
    node_->get_parameter(prefix + "planning_horizon", config.planning_horizon);
    node_->get_parameter(prefix + "dist0", config.optimizer_dist0);
    node_->get_parameter(prefix + "obstacles_inflation", config.obstacles_inflation);
    node_->get_parameter(prefix + "virtual_ceil_height", config.virtual_ceil_height);
    node_->get_parameter(prefix + "column_fill_enabled", config.column_fill_enabled);
    node_->get_parameter(prefix + "column_fill_max_height", config.column_fill_max_height);
    node_->get_parameter(prefix + "cloud_inflate_z_step", config.cloud_inflate_z_step);

    planner_mode_configs_[mode] = config;
  }

  bool EGOReplanFSM::applyPlannerMode(const std::string &mode, std::string &message)
  {
    ...
    auto config_iter = planner_mode_configs_.find(normalized_mode);
    if (config_iter == planner_mode_configs_.end())
    {
      message = "Unknown planner mode: " + mode + ". Use ONE, TWO, THREE, FOUR, or NORMAL.";
      return false;
    }

    const PlannerModeConfig &config = config_iter->second;
    if (config.max_vel <= 0.0 || config.max_acc <= 0.0 || config.ctrl_pt_dist <= 0.0 ||
        config.planning_horizon <= 0.0 || config.optimizer_dist0 < 0.0 ||
        config.obstacles_inflation < 0.0 || config.column_fill_max_height < 0.0 ||
        config.cloud_inflate_z_step < 0)
    {
      message = "Planner mode " + normalized_mode + " has invalid non-positive parameters.";
      return false;
    }

    planning_horizen_ = config.planning_horizon;
    planner_manager_->setDynamicParams(config.max_vel,
                                       config.max_acc,
                                       config.ctrl_pt_dist,
                                       config.planning_horizon,
                                       config.optimizer_dist0,
                                       config.obstacles_inflation,
                                       config.virtual_ceil_height,
                                       config.column_fill_enabled,
                                       config.column_fill_max_height,
                                       config.cloud_inflate_z_step);

    message = "Planner mode set to " + normalized_mode +
              " (max_vel=" + std::to_string(config.max_vel) +
              ", max_acc=" + std::to_string(config.max_acc) +
              ", horizon=" + std::to_string(config.planning_horizon) +
              ", virtual_ceil_height=" + std::to_string(config.virtual_ceil_height) + ")";
    return true;
  }
  ...
  void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
  {
    ...
    state_msg.data = static_cast<int>(exec_state_);
    exec_state_pub_->publish(state_msg);
