...
namespace ego_planner
{
  ...
  void EGOPlannerManager::setDynamicParams(double max_vel,
                                           double max_acc,
                                           double ctrl_pt_dist,
                                           ...
                                           int cloud_inflate_z_step)
  {
    ...
    pp_.ctrl_pt_dist = ctrl_pt_dist;
    pp_.planning_horizen_ = planning_horizon;

    if (bspline_optimizer_)
    {
      bspline_optimizer_->setDynamicParams(max_vel, max_acc, optimizer_dist0);
    }

    if (grid_map_)
    {
      grid_map_->setDynamicParams(obstacles_inflation, virtual_ceil_height, column_fill_enabled, column_fill_max_height, cloud_inflate_z_step);
    }
  }
